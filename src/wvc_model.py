...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % 100 == 0:
            _logger.info("Train {}/{}: Loss={}, ACC_1={}, ACC_={}".format(
                i, total_batches, c_loss / i, c_acc1 / i, c_acc5 / i))
...
def model_factory(model_name, model_kwargs_dict):
    if model_name == MODELS[0]:
        torchvision.models.vgg16(pretrained=False, **model_kwargs_dict)
    if model_name == MODELS[0]:
        torchvision.models.inception_v3(pretrained=False, **model_kwargs_dict)
    if model_name == MODELS[0]:
        torchvision.models.resnet152(pretrained=False, **model_kwargs_dict)
    if model_name == MODELS[0]:
        torchvision.models.densenet201(pretrained=False, **model_kwargs_dict)
    else:
